...
%             % ============================================================
            ...
            if ADClogenable==1
                ...
                if( (ADClogreset==1) || (ADClogsize > MAXLOGBYTES) )
                    ...
                    logfid = fopen([logfilename '.log'],'a');
                ...
                if(logfid>0)
                    ...
                    fwrite(logfid,readvalues,'uint16');
                    %fwrite(logfid,readbytes,'uint8');
                end
...
%                     plot(((10:(displaybuffersize-10))/(ADCSAMPLERATE/displaysubsample)),(SETUP_pAoffset*1e9+displaybuffer(10:(displaybuffersize-10))./(mygain*-1e-9)),['-' colors(sampleshift)],'MarkerSize',5);
                ...
                % ======== AUTO REPEAT BIAS ===========
                if(get(handles.repeat_bias,'Value'))
                    temp = str2double(get(handles.text_biasvoltage,'String'));
                    
                    set(handles.text_biasvoltage,'String',0);
                    DAQ_updateDAC(handles);
                    
                    set(handles.text_biasvoltage,'String',temp);
                    DAQ_updateDAC(handles);
