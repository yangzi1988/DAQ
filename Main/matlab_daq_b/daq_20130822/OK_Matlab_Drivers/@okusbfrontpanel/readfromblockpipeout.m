...
if (psize == bsize),
	...
	for k = 1:fix(bsize/psize),
        % JKR changed next line from x to buf
        %size(buf)
        %[pv,buf] = calllib('okFrontPanel', 'okFrontPanel_ReadFromBlockPipeOut', obj.ptr, epaddr, blksize, psize, pv);
        %size(buf)
        calllib('okFrontPanel', 'okFrontPanel_ReadFromBlockPipeOut', obj.ptr, epaddr, blksize, psize, pv);
        %epvalue(kk) = buf(1:psize);
        epvalue(kk) = get(pv,'value');
		kk = kk+psize;
	end
	psize_last = mod(bsize,psize);
	kk = kk(1:psize_last);
    % JKR changed this next line from size_last to psize_last and x to buf
	%[x,pv] = calllib('okFrontPanel', 'okFrontPanel_ReadFromBlockPipeOut', obj.ptr, epaddr, blksize, size_last, pv);
    [pv,buf] = calllib('okFrontPanel', 'okFrontPanel_ReadFromBlockPipeOut', obj.ptr, epaddr, blksize, psize_last, pv);
